...
class DivyBikeShareCalc():
    ...
    def __init__(self):
        ...
        while True:
            ...
            if df is None or df.empty:
                print('=' * 80)
                print("DataFrame is either None or empty. Please check again")
                print('=' * 80)
                continue
            else:
                self.time_stats(df)
            ...
            restart = input('\nWould you like to restart? Enter y or anything else.\n')
            if restart.lower() != 'y':
                break

    #Added Enum to avoid repetition
    def choice_enum_iteration(self, **kwargs):
        """
        Added this block to help with choice enumeration
        """
        select_statement = ""
        while True:
    ...
    def list_sources(self, dir):
        """
        Sources iterated from the landing directory
        """
    ...
    def get_filters(self):
        ...
        city_csv_source = self.list_sources(self.DIRECTORY)

        print(city_csv_source)

        cities = list(city_csv_source.keys())

        print("\nHere are the available cities with a valid csv:\n")

        city_choice = self.choice_enum_iteration(name='city details', calclist=cities, exitchar='x', selectall='a')
        ...
        month = self.choice_enum_iteration(name='month', calclist=months, exitchar='x', selectall='a')

        # get user input for day of week (all, monday, tuesday, ... sunday)

        days_of_week = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']

        day = self.choice_enum_iteration(name='day', calclist=days_of_week, exitchar='x', selectall='a')
    ...
    def time_stats(self, df):
        ...
        choice = self.choice_enum_iteration(name='choices', calclist=choices, exitchar='x')
    ...
    def station_stats(self, df):
        ...
        choices = ['most commonly used start station', 'most commonly used end station', 'most frequent combination of start station and end station trip']
